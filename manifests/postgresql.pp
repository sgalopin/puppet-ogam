class ogam::postgresql {
    ...
    class { 'postgresql::globals':
		...
		file_line { 'client_min_messages':
			...
			line   => 'client_min_messages = debug5', # https://www.postgresql.org/docs/9.6/static/runtime-config-logging.html
		}->
		file_line { 'log_min_messages':
			ensure => present,
			path   => '/etc/postgresql/9.6/main/postgresql.conf',
			match  => 'log_min_messages = .*',
			line   => 'log_min_messages = debug5',
		}->
		file_line { 'log_min_error_statement':
			ensure => present,
			path   => '/etc/postgresql/9.6/main/postgresql.conf',
			match  => 'log_min_error_statement = .*',
			line   => 'log_min_error_statement = debug5',
		}
